---
title: "2˚ Shifts Richness change analysis"
output: html_notebook
---

This code loads in latitude versus area data tables from annotate HPC and adds in calculations for changes in species richness based on SAR from Levin et al. 2009. It does not output figures, which you can find in Figure3_5 code. It should be run after degree_shifts_projected_2degrees.Rmd which calculates area at latitude.
...
shelf_areas[, percent_change_richness_mean := (original_spp_predicted_38-data.table::shift(original_spp_predicted_38, type = "lag"))/data.table::shift(original_spp_predicted_38, type = "lag"), .(region,hemisphere)]

shelf_areas[, percent_change_richness_min := (original_spp_predicted_175-data.table::shift(original_spp_predicted_175, type = "lag"))/data.table::shift(original_spp_predicted_175, type = "lag"), .(region,hemisphere)]

shelf_areas[, percent_change_richness_max := (original_spp_predicted_62-data.table::shift(original_spp_predicted_62, type = "lag"))/data.table::shift(original_spp_predicted_62, type = "lag"), .(region,hemisphere)]

shelf_areas[,spp_per_change_significant_mean := ifelse(percent_change_richness_mean >= 1, "Gain",ifelse(percent_change_richness_mean <= -0.5, "Loss","NA"))]

#new data table with only these values
shelf_areas_highlight_mean <- shelf_areas[spp_per_change_significant_mean == "Gain" | spp_per_change_significant_mean == "Loss"]
...
#mean
mean(shelf_areas$change_spp_predicted_38, na.rm = T)


shelf_areas_fullstats <- shelf_areas

#save data table
saveRDS(shelf_areas_fullstats, file = here::here("output","2_degree_latitude","shelf_areas_fullstats.rds"))
  
```

How well correlated are percent changes across z values?
```{r percent change across z}
cor(shelf_areas_fullstats[,.(percent_change_richness_min, percent_change_richness_mean, percent_change_richness_max)], use = "complete.obs")
```

How well correlated are species gains and losses across z values?
```{r percent change across z}
cor(shelf_areas_fullstats[,.(change_spp_predicted_175, change_spp_predicted_38, change_spp_predicted_62)], use = "complete.obs")
```

Figure for spp richness change by latitude
```{r spp change lat}
(spp_change_latitude_min  <- ggplot(data = shelf_areas) +
  geom_col(aes(x=latitude_end, y=change_spp_predicted_175, fill = change_dir)) +
 labs(x=paste0("Latitude ","\u00B0","E"), y = "Change in Richness\nwith 2\u00B0 Shift") +
     scale_fill_viridis_d(option = "A", begin = 0.3, end = 0.7, name = "Direction of\nRichness Change\nWith 2\u00B0 Shift\nz=0.175", labels = c("Negative", "Positive")) +
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = 0) +
  coord_flip() +
  theme_classic() +
...
(spp_change_latitude_mean  <- ggplot(data = shelf_areas) +
  geom_col(aes(x=latitude_end, y=change_spp_predicted_38, fill = change_dir)) +
 labs(x=paste0("Latitude ","\u00B0","E"), y = "Change in Richness\nwith 2\u00B0 Shift") +
     scale_fill_viridis_d(option = "A", begin = 0.3, end = 0.7, name = "Direction of\nRichness Change\nWith 2\u00B0 Shift\nz=0.38", labels = c("Negative", "Positive")) +
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = 0) +
  coord_flip() +
  theme_classic() +
...
(spp_change_latitude_max  <- ggplot(data = shelf_areas) +
  geom_col(aes(x=latitude_end, y=change_spp_predicted_62, fill = change_dir)) +
 labs(x=paste0("Latitude ","\u00B0","E"), y = "Change in Richness\nwith 2\u00B0 Shift") +
     scale_fill_viridis_d(option = "A", begin = 0.3, end = 0.7, name = "Direction of\nRichness Change\nWith 2\u00B0 Shift\nz=0.62", labels = c("Negative", "Positive")) +
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = 0) +
  coord_flip() +
  theme_classic() +
