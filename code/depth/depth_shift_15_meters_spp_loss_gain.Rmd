...
Calculate change in species richness with 15 m depth shift. This code follows bathy_shelf_byLME.Rmd which calculates LME statistics for how area changes over depth. 
...
Pull in Data (updated with etopo1 on April 26, 2022)
```{r pull in data}
...
#to skip this time consuming step later
writeRaster(etopo1_shelf_raster, filename = here::here("output","intermediate_data","etopo1_shelf_raster.grd"))
...
mean(etopo1_shelf_raster$layer)
hist(etopo1_shelf_raster$layer)

cellStats(etopo1_shelf_raster, stat = 'mean')
hist(etopo1_shelf_raster)
```

#Check why California has more depths than it did in the past
...
percent_change_richness_levin = numeric(),
percent_change_richness_175 = numeric(),
percent_change_richness_38 = numeric(),
percent_change_richness_62 = numeric(),
...
lme_spp_count_area_change_min <- vector("list", length = nrow(LME_spdf))
lme_spp_count_area_change_mean <- vector("list", length = nrow(LME_spdf))
lme_spp_count_area_change_max <- vector("list", length = nrow(LME_spdf))
...
#percent change
LME_bathy_bydepth_complete_15mbins_single[, percent_change_richness_levin := (original_spp_predicted_levin-data.table::shift(original_spp_predicted_levin, type = "lag"))/data.table::shift(original_spp_predicted_levin, type = "lag"), .(lme_name)]

LME_bathy_bydepth_complete_15mbins_single[, percent_change_richness_175 := (original_spp_predicted_175-data.table::shift(original_spp_predicted_175, type = "lag"))/data.table::shift(original_spp_predicted_175, type = "lag"), .(lme_name)]

LME_bathy_bydepth_complete_15mbins_single[, percent_change_richness_38 := (original_spp_predicted_38-data.table::shift(original_spp_predicted_38, type = "lag"))/data.table::shift(original_spp_predicted_38, type = "lag"), .(lme_name)]

LME_bathy_bydepth_complete_15mbins_single[, percent_change_richness_62 := (original_spp_predicted_62-data.table::shift(original_spp_predicted_62, type = "lag"))/data.table::shift(original_spp_predicted_62, type = "lag"), .(lme_name)]
  ...
  lme_spp_count_area_sum_plots[[i]] <- ggplot(data = LME_bathy_bydepth_complete_15mbins_single, aes(x = depth_15s)) +
    ...
    scale_fill_viridis_d(option = "A", begin = 0.7, end = 0.3, name = paste0("LME: ", LME_area_spp_change_stats[i, "lme_number"],"\n", LME_area_spp_change_stats[[i, "lme_name"]],"\n\nRichness Change with\n15m Depth Shift"), breaks = c(1,-1), labels = c("Gain", "Loss")) +
    labs(x = "Depth  (15 m bins)", y = expression(Delta~'Species Richness'), title = paste0("LME ",LME_area_spp_change_stats[i,lme_number],": ",LME_area_spp_change_stats[i,lme_name])) +
#    geom_hline(yintercept = 0) +
  ...
  #plots for min, mean, and max 
  lme_spp_count_area_change_min[[i]] <- ggplot(data = LME_bathy_bydepth_complete_15mbins_single, aes(x = depth_15s)) +
    geom_col(aes(y = percent_change_richness_175*100, fill=spp_change_pos_neg_175_discrete), alpha = 0.5) +
    scale_fill_viridis_d(option = "A", begin = 0.7, end = 0.3, name = paste0("LME: ", LME_area_spp_change_stats[i, "lme_number"],"\n", LME_area_spp_change_stats[[i, "lme_name"]],"\n\nPercent Richness Change with\n15m Depth Shift"), breaks = c("Gain","Loss"), labels = c("Gain", "Loss")) +
    labs(x = "Depth  (15 m bins)", y = expression('Percent'~Delta~'Species Richness z = 0.175'), title = paste0("LME ",LME_area_spp_change_stats[i,lme_number],": ",LME_area_spp_change_stats[i,lme_name])) +
#    geom_hline(yintercept = 0) +
    theme_classic() +
    scale_x_continuous(expand = c(0,0)) +
    scale_y_continuous(expand = c(0,0)) +
  ...
  #plot containing distribution of mean z value
  lme_spp_count_area_change_mean[[i]] <- ggplot(data = LME_bathy_bydepth_complete_15mbins_single, aes(x = depth_15s)) +
    geom_col(aes(y = percent_change_richness_38*100, fill=spp_change_pos_neg_38_discrete), alpha = 0.5) +
    scale_fill_viridis_d(option = "A", begin = 0.7, end = 0.3, name = paste0("LME: ", LME_area_spp_change_stats[i, "lme_number"],"\n", LME_area_spp_change_stats[[i, "lme_name"]],"\n\nPercent Richness Change with\n15m Depth Shift"), breaks = c("Gain","Loss"), labels = c("Gain", "Loss")) +
    labs(x = "Depth  (15 m bins)", y = expression('Percent'~Delta~'Species Richness z = 0.38'), title = paste0("LME ",LME_area_spp_change_stats[i,lme_number],": ",LME_area_spp_change_stats[i,lme_name])) +
#    geom_hline(yintercept = 0) +
    theme_classic() +
    scale_x_continuous(expand = c(0,0)) +
    scale_y_continuous(expand = c(0,0)) +
  ...
  #plot containing distribution of max z value
  lme_spp_count_area_change_max[[i]] <- ggplot(data = LME_bathy_bydepth_complete_15mbins_single, aes(x = depth_15s)) +
    geom_col(aes(y = percent_change_richness_62*100, fill=spp_change_pos_neg_62_discrete), alpha = 0.5) +
    scale_fill_viridis_d(option = "A", begin = 0.7, end = 0.3, name = paste0("LME: ", LME_area_spp_change_stats[i, "lme_number"],"\n", LME_area_spp_change_stats[[i, "lme_name"]],"\n\nPercent Richness Change with\n15m Depth Shift"), breaks = c("Gain","Loss"), labels = c("Gain", "Loss")) +
    labs(x = "Depth  (15 m bins)", y = expression('Percent'~Delta~'Species Richness z = 0.62'), title = paste0("LME ",LME_area_spp_change_stats[i,lme_number],": ",LME_area_spp_change_stats[i,lme_name])) +
#    geom_hline(yintercept = 0) +
    theme_classic() +
    scale_x_continuous(expand = c(0,0)) +
    scale_y_continuous(expand = c(0,0)) +
...
saveRDS(lme_spp_count_area_change_min, file = here::here("output","compiled_plots", "lme_spp_count_area_change_min.rds"))
saveRDS(lme_spp_count_area_change_mean, file = here::here("output","compiled_plots", "lme_spp_count_area_change_mean.rds"))
saveRDS(lme_spp_count_area_change_max, file = here::here("output","compiled_plots", "lme_spp_count_area_change_max.rds"))
...
#min
ggsave(path = here::here("Figures","compiled_plots"), filename = "arranged_LME_sppchange_multiple_min.pdf", marrangeGrob(lme_spp_count_area_change_min, ncol = 2, nrow = 3), width = 8.5, height = 11, unit = "in")

#mean
ggsave(path = here::here("Figures","compiled_plots"), filename = "arranged_LME_sppchange_multiple_mean.pdf", marrangeGrob(lme_spp_count_area_change_mean, ncol = 2, nrow = 3), width = 8.5, height = 11, unit = "in")

#max
ggsave(path = here::here("Figures","compiled_plots"), filename = "arranged_LME_sppchange_multiple_max.pdf", marrangeGrob(lme_spp_count_area_change_max, ncol = 2, nrow = 3), width = 8.5, height = 11, unit = "in")
...
Correlations between 0.175,  0.38,  and 0.62
```{r}
cor(LME_bathy_bydepth_complete_15mbins[,.(percent_change_richness_175, percent_change_richness_38, percent_change_richness_62)], use = "complete.obs")
```
