...
This code produces LME_bathy_statistics.rds which lists LME name, number & and statistics about that LME including:
- skew
- modality
- diptest
- mid-depth
...
The diptest requires a Monte Carlo simulation, and therefore takes a while to run. Therefore, we ran this on a HPC. 
...
#etopo1_shelf_df <- readRDS(here::here("raw_data/ETOPO1/etopo1_shelf_df.rds"))
#bring in bathy for shelf regions (just pull in raster below)

#LMEs: http://lme.edc.uri.edu/index.php/digital-data/113-lme-polygon-boundaries
...
```{r rasterize}
#proj <- crs(LME_spdf)
#etopo1_shelf_raster <- rasterFromXYZ(etopo1_shelf_df, crs = proj)

#just pull in raster that you saved
etopo1_shelf_raster <- raster(here::here("output","intermediate_data","etopo1_shelf_raster.gri"))


#make figure of shelves
  #png("etopo1_shelf_bathy.png", height=5, width=7, units="in", res=300)
  #plot(etopo1_shelf_raster)
  #dev.off()
...
mean(etopo1_shelf_raster$z)
hist(etopo1_shelf_raster$z)

cellStats(etopo1_shelf_raster, stat = 'mean')
hist(etopo1_shelf_raster)
```

Depth distribution for Antarctica and Arctic specifically
```{r Antarctic & Arctic}
#crop bathy layer to LME subset
bathy_arctic_extent <- crop(etopo1_shelf_raster, extent(LME_poles[2,]))
...
#crop bathy layer to LME subset
bathy_baltic_extent <- crop(etopo1_shelf_raster, extent(LME_baltic))
...
#crop bathy layer to LME subset
bathy_aleutian_extent <- crop(etopo1_shelf_raster, extent(LME_aleutian))
...
This takes too long on local machine, so instead I ran on annotate (HPC). Skip to 585.

```{r pull LME_bathy statistics from annotate}

LME_bathy_statistics <- readRDS("~/Documents/grad_school/Rutgers/Repositories/shelf_habitat_distribution/output/files_back_from_annotate/LME_bathy_statistics.rds")
```
...
for (i in 1:nrow(LME_bathy_statistics)) {
 ...
 #   LME_points <- spTransform(LME_points, CRS())
  ...
  #To get a polygon that surrounds cells that are not NA
    ...
    # convert to polygons (you need to have package 'rgeos' installed for this to work)
    pp <- rasterToPolygons(r, dissolve=TRUE) #longer step
  ...
  #  geom_path(data = LME_reduced_df, aes(x = long_shift, y = lat, group = group), color = "black") +
    labs(title = paste0("LME ", LME_bathy_statistics[i,lme_number], "\n", LME_bathy_statistics[i,lme_name])) + #switch direction of / for annotate
  ...
  freq_table_raster[,area_rounddown := round(prop_area * 1000000, 0)] #five sig figs should be enough

  #empty vector
  LME_data_vector <- vector()

  #populate vector where each depth is counted the same # of times as area_rounddown (proportional number of times that depth is present in raster)
...
#    geom_smooth(method = "gam", se = F, color = "black", size = 1) +
  ...
  #uniformity (here I'm using list of cell depths assuming all cells are the same size. This isn't ideal, but okay for just this test because cell size doesn't actually vary much)
  ...
  #Since the p-value is less than .05, we reject the null hypothesis. We have sufficient evidence to say that the sample data does not come from uniform distribution
  
  #unimodality
  #Computes Hartigans’ dip test statistic for testing unimodality, and additionally the modal interval.
  
   diptest <- dip.test(LME_data_vector, simulate.p.value = TRUE, B = 2000)
#this takes a while, probably more than 30 minutes, started at 2:39
...
LME_bathy_statistics_full <- LME_bathy_statistics

write.csv(LME_bathy_statistics_full,here::here("output", "LME_bathy_statistics_full.csv"))
saveRDS(LME_bathy_statistics_full, here::here("output","LME_bathy_statistics_full.rds"))
...
LME_bathy_statistics_altskew <- LME_bathy_statistics_full
