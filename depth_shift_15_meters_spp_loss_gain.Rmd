...
LME_bathy_bydepth_complete_15mbins <- data.table(depth_15s = numeric(), area_summed_15mbins = numeric(), original_spp_predicted_levin = numeric(), change_area = numeric(), change_spp_predicted_levin = numeric(), spp_change_pos_neg_levin = factor(), lme_number = numeric(), lme_name = numeric())
...
for (i in 1:nrow(LME_area_spp_change_stats)) {
  ...
  LME_bathy_bydepth_complete_15mbins_single <- unique(LME_bathy_bydepth_complete[,.(depth_15s, area_summed_15mbins)])
  
  LME_bathy_bydepth_complete_15mbins_single[,original_spp_predicted_levin := 16.18*area_summed_15mbins^0.226]
  ...
  #change in area
    LME_bathy_bydepth_complete_15mbins_single[, change_area :=  area_summed_15mbins -  data.table::shift(area_summed_15mbins, n = 1, type = "lag")] #change in species from previous bin to this bin?
  #change in spp richness
  LME_bathy_bydepth_complete_15mbins_single[, change_spp_predicted_levin :=  original_spp_predicted_levin -  data.table::shift(original_spp_predicted_levin, n = 1, type = "lag")] #change in species from previous bin to this bin?

  LME_bathy_bydepth_complete_15mbins_single[, spp_change_pos_neg_levin := ifelse(change_spp_predicted_levin > 0, "Positive","Negative")] #for coloring
...
saveRDS(LME_bathy_bydepth_complete_15mbins, here::here("table_outputs","LME_bathy_bydepth_complete_15mbins.rds"))
