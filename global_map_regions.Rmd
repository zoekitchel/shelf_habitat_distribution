...
(global_LME_classification_map <- ggplot() + 
  geom_polygon(data = LME_spdf_working_proj.dt, aes(x = long, y = lat, group = group, fill = as.factor(type)), color = "darkslategrey", size = 0.03, alpha = 0.6) +
  scale_fill_viridis_d(option = "A", begin = 0.4, end = 0.9, labels = c("Shallow Dominant: 19%", "Multimodal: 69%", "Mid Dominant: 12%")) +
...
ggsave(global_LME_classification_map, filename = "map_global_type.eps")
```
