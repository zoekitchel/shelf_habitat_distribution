...
west_pac_shelf_areas[, latitude_start := as.numeric(V1)][, latitude_end := as.numeric(V1)][, area_rasterarea := as.numeric(V1)][, area_equalareaproj := as.numeric(V1)][, V1 := NULL]
...
for (i in 1:(length(west_pac_south_latitudes)-1)) {
  ...
  west_pac_shelf_areas[i, "latitude_start"] <- west_pac_south_latitudes[i]
  west_pac_shelf_areas[i, "latitude_end"] <- west_pac_south_latitudes[i+1]
  west_pac_shelf_areas[i, "area_equalareaproj"] <- 0
  ...
  #  If x is a SpatialPolygons* object: area of each spatial object in squared meters if the CRS is longitude/latitude, or in squared map units (typically meter)
    ...
    #delete NAs from vector of all raster cells
    ##NAs lie outside of the rastered region, can thus be omitted
    cell_size_sp<-cell_size_sp[!is.na(segment_south.sp.EA)]
    #compute area [km2] of all cells in geo_raster
    segment_area_equalarea <-length(cell_size_sp)*median(cell_size_sp)
