...
etopo_shelf_dt_north <- etopo_shelf_dt[y >= 0,][,habitat := ifelse(is.na(z), 0,1)] #restrict to latitudes equal to or above 0 (northern hemisphere), and then create new column where any grid cells assigned a depth are now reclassified to 1
setorder(etopo_shelf_dt_north, y)
etopo_shelf_dt_south <- etopo_shelf_dt[y <= 0,][,habitat := ifelse(is.na(z), 0,1)] #restrict to latitudes equal to or below 0 (southern hemisphere), and then create new column where any grid cells assigned a depth are now reclassified to 1
setorder(etopo_shelf_dt_south, -y)
```

Binning data by 1 ˚
```{r}
etopo_shelf_dt_north_binned <- etopo_shelf_dt_north[,sum(habitat) , .(gr=cut(y,breaks=seq(0,90,by = (1))))] #sum # of cells in each y bin of 1˚
etopo_shelf_dt_south_binned <- etopo_shelf_dt_south[,sum(habitat) , .(gr=cut(y,breaks=seq(0,-90,by = (-1))))]
```
...
Testing to see if projection worked to begin with
```{r}
f <- freq(etopo_shelf_raster, useNA='no')
apc <- prod(res(etopo_shelf_raster))
...
west_pac_north_latitudes <- seq(0, ymax(west_pac_spdf_shift_mask), by = 1)
west_pac_south_latitudes <- seq(-1, ymin(west_pac_spdf_shift_mask), by = -1)
...
for (i in 1:(length(west_pac_north_latitudes)-1)) {
  ...
  #raster segment
  segment_north <- crop(west_pac_spdf_shift_mask, extent(north_extent))
  
  #convert segment to polygon from raster
  segment_north.sp <- rasterToPolygons(segment_north)
  
  
  if(all(is.na(values(segment_north)))) { #if there's no shelf area within a bin
  ...
  west_pac_shelf_areas[i, "`area_raster::area`"] <- 0
  ...
  segment_north.sp.EA <- spTransform(segment_north.sp, CRSobj = equalareaprojection)
    ...
    cell_size_sp<-area(segment_north.sp.EA, na.rm=TRUE, weights=FALSE)

    #compute area [km2] of all polygons in shape file
    segment_area_equalarea <-length(cell_size_sp)*median(cell_size_sp)

  west_pac_shelf_areas[i, "area_equalareaproj"] <- segment_area_equalarea/(1000^2)
  
  #and then plain and simple also using rgeos::gArea
  
  segment_north.sp.EA@data$area_km2 <- gArea(segment_north.sp.EA, byid = TRUE) / (1000^2)
  
  
  west_pac_shelf_areas[i, "area_rgeos_gArea"] <- sum(segment_north.sp.EA@data$area_km2)
...
for (i in 1:(length(west_pac_south_latitudes)-1)) {
  ...
  #raster segment
  segment_south <- crop(west_pac_spdf_shift_mask, extent(south_extent))
  
    #convert raster segment to polygon
  segment_south.sp <- rasterToPolygons(segment_south)
  ...
  segment_south.sp.EA <- spTransform(segment_south.sp, CRSobj = equalareaprojection)
    ...
    cell_size_sp<-area(segment_south.sp.EA, na.rm=TRUE, weights=FALSE)

    #compute area [km2] of all cells in geo_raster
    segment_area_equalarea <-length(cell_size_sp)*median(cell_size_sp)

  west_pac_shelf_areas[i+(length(west_pac_north_latitudes)-1), "area_equalareaproj"] <- segment_area_equalarea/(1000^2)
  
    #and then plain and simple also using rgeos::gArea
  west_pac_shelf_areas[i+(length(west_pac_north_latitudes)-1), "area_rgeos_gArea"] <- gArea(segment_south.sp.EA)/(1000^2)
...
ggplot(data = west_pac_shelf_areas) +
  geom_point(aes(x = latitude_start, y = area_rgeos_gArea), color = "purple") +
  geom_point(aes(x = latitude_start, y = area_rasterarea), color = "green") +
  geom_point(aes(x = latitude_start, y = area_equalareaproj), color = "red") +
...
between 1 and Inf = at least 2 fold increase (note I classified any change from 0 to something as NOT a significant change, should return to this)

between -0.5 and -inf = atleast 2 fold decrease
...
east_pac_north_latitudes <- seq(0, ymax(east_pac_spdf_shift_mask), by = 1)
east_pac_south_latitudes <- seq(-1, ymin(east_pac_spdf_shift_mask), by = -1)
...
#all chunks for east pacific
west_atl_north_latitudes <- seq(0, ymax(west_atl_spdf_mask), by = 1)
west_atl_south_latitudes <- seq(-1, ymin(west_atl_spdf_mask), by = -1)
...
#all chunks for east pacific
east_atl_north_latitudes <- seq(0, ymax(east_atl_spdf_nobuf_mask), by = 1)
east_atl_south_latitudes <- seq(-1, ymin(east_atl_spdf_nobuf_mask), by = -1)
...
#all chunks for east pacific
west_ind_north_latitudes <- seq(0, ymax(west_ind_spdf_mask), by = 1)
west_ind_south_latitudes <- seq(-1, ymin(west_ind_spdf_mask), by = -1)
...
#all chunks for east pacific
east_ind_north_latitudes <- seq(0, ymax(east_ind_spdf_mask), by = 1)
east_ind_south_latitudes <- seq(-1, ymin(east_ind_spdf_mask), by = -1)
...
save(east_ind_shelf_areas, west_ind_shelf_areas, east_atl_shelf_areas, west_atl_shelf_areas, east_pac_shelf_areas, west_pac_shelf_areas ,file = "shelf_areas_by_1latitude.RData")
