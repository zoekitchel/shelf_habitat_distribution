...
```{r missing from LMEs}
#Papua New Guinea Extent
#1.0240205546580048, 132.62031168164683
#-8.785049033825267, 132.4445304387777
#-12.069257731776018, 156.52656071184586
#-1.0852349879052885, 154.94452952602384


papua_extent <- extent(132.4445304387777,162.38243906602315, -12.069257731776018, 1.0240205546580048)
papua_raster <- crop(etopo_shelf_raster, papua_extent)

#Southern Sumatra Coast Extent


southern_sumat_extent <- extent(94.12421874100268, 117, -9.392555887692712, 5.235523767930577)
southern_sumat_raster <- crop(etopo_shelf_raster, southern_sumat_extent)

#minus LME 36 & 35
LME_35_36 <- LME_spdf[LME_spdf$LME_NUMBER %in% c(35:36),]
LME_35_36_extent <- extent(LME_35_36)
LME_35_36_crop <- crop(etopo_shelf_raster, LME_35_36_extent)
LME_35_36_raster <- mask(LME_35_36_crop, LME_35_36)
lme_35_36_raster_sumat_overlap <- crop(LME_35_36_raster, extent(southern_sumat_raster))

#extend extent of lme_35_36_raster_sumat_overlap
lme_35_36_raster_sumat_full_extent <- extend(lme_35_36_raster_sumat_overlap, extent(southern_sumat_extent))

#reclassify all values as 
LME_35_36_raster_5s <- reclassify(lme_35_36_raster_sumat_full_extent, c(-Inf, Inf, 5))

sum_westind_sumat <- sum(LME_35_36_raster_5s,southern_sumat_raster, na.rm = T)

southern_sumatra_minus_35_36 <- reclassify(sum_westind_sumat, c(-Inf,0,NA, 1,1,1, 2,Inf,NA))


#Outer Phillipines Extent

philippines_extent <- extent(121.01874933852636, 130.68671769632746, 4.359706039079976, 20.60413268328657)
philippines_raster <- crop(etopo_shelf_raster, philippines_extent)
```

```{r merging LMEs}
west_pac <- c(42, 41, 40, 38, 37, 35, 36, 47, 49, 48, 50, 51, 52, 53, 1, 54, 56, 57, 58, 20, 39) #and phillipines and Papua New Guinea
...
east_ind <- c(42, 43, 44, 45, 34, 38, 39) #AND ADD SUMATRA
...
#east indian ocean
east_ind_spdf_mask_full <- merge(east_ind_spdf_mask_1s, southern_sumatra_minus_35_36)

#western pacific ocean
west_pac_spdf_mask_full <- merge(papua_raster, west_pac_spdf_shift_mask_1s)
west_pac_spdf_mask_full <- merge(philippines_raster, west_pac_spdf_mask_full)
```
...
east_ind_spdf_mask_full

```{r split raster for eastern indian}
north_extent <- c(xmin(east_ind_spdf_mask_full), xmax(east_ind_spdf_mask_full), 0, ymax(east_ind_spdf_mask_full))
south_extent <- c(xmin(east_ind_spdf_mask_full), xmax(east_ind_spdf_mask_full), ymin(east_ind_spdf_mask_full), 0)

#crop east_ind raster above and below 0
east_ind_spdf_shift_agg_north <- crop(east_ind_spdf_mask_full, extent(north_extent))

east_ind_spdf_shift_agg_south <- crop(east_ind_spdf_mask_full, extent(south_extent))

#unfortunately, I think I may have to just do this manually (ugly, I know)

#all chunks for east indian
east_ind_north_latitudes <- seq(0, ymax(east_ind_spdf_mask_full), by = 2)
east_ind_south_latitudes <- seq(0, ymin(east_ind_spdf_mask_full), by = -2)
...
for (i in 1:(length(east_ind_north_latitudes)-1)) {
  #setting up extent for slicing by min and max longitudes, and i to i+1 latitudes
  north_extent <- c(xmin(east_ind_spdf_mask_full), xmax(east_ind_spdf_mask_full), east_ind_north_latitudes[i], east_ind_north_latitudes[i+1])
  
  #crop raster segement based on bin extent
  segment_north <- crop(east_ind_spdf_mask_full, extent(north_extent))
...
for (i in 1:(length(east_ind_south_latitudes)-1)) {
  south_extent <- c(xmin(east_ind_spdf_mask_full), xmax(east_ind_spdf_mask_full), east_ind_south_latitudes[i+1], east_ind_south_latitudes[i]) #order= xmin, xmax, ymin, ymax)
  
  #raster segment
  segment_south <- crop(east_ind_spdf_mask_full, extent(south_extent))
...
for (i in 1:length(shelf_areas)) {
  ...
  this_shelf_area <- get(shelf_areas[i])
    ...
    regional_statistics[region == unique(regional_statistics[,region])[i] & hemisphere == "Northern", "contractions"] <- round(this_shelf_area[latitude_end > 0 & change_above_2fold == -1,.N]/nrow(this_shelf_area[latitude_end > 0 & !is.na(percent_change)]),2)*100 #northern hemisphere contraction
    
    regional_statistics[region == unique(regional_statistics[,region])[i] & hemisphere == "Southern", "contractions"] <- round(this_shelf_area[latitude_end < 0 & change_above_2fold == -1,.N]/nrow(this_shelf_area[latitude_end < 0 & !is.na(percent_change)]),2)*100 #southern hemisphere contraction
    
    regional_statistics[region == unique(regional_statistics[,region])[i] & hemisphere == "Northern", "expansions"] <- round(this_shelf_area[latitude_end > 0 & change_above_2fold == 1,.N]/nrow(this_shelf_area[latitude_end > 0 & !is.na(percent_change)]),2)*100 #northern hemisphere expansion
    
    regional_statistics[region == unique(regional_statistics[,region])[i] & hemisphere == "Southern", "expansions"] <- round(this_shelf_area[latitude_end < 0 & change_above_2fold == 1,.N]/nrow(this_shelf_area[latitude_end < 0 & !is.na(percent_change)]),2)*100 #southern hemisphere expansion
