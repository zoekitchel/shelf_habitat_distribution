...
-34, 38, 43, 44, 45
  ...
  -** not sure how to split eastern Indian from western pacific 
  -3/8 update
  -42, 43, 44, 45, western Sumatra, 39, 34, 38

Western Pacific (6)
1,	35,	36,	37,	39,	40,	41,	42,	46,	47,	48,	49,	50,	51,	52,	53,	54,	56,	57,	65

  -3/8 update
  -42, 41, 40, Papua New Guinea, etc, 38, 37, 35, 36, 47, 49, 48, 50, 51, 52, 53, 1,     54, 56, 57, 58, 20, 39, outer phillipines
...
papua_extent <- extent(132.4445304387777,156.52656071184586, -12.069257731776018, 1.0240205546580048)
...
#minus LME 36
LME_36 <- LME_spdf[LME_spdf$LME_NUMBER == 36,]
LME_36_extent <- extent(LME_36)
LME_36_crop <- crop(etopo_shelf_raster, LME_36_extent)
LME_36_raster <- mask(LME_36_crop, LME_36)
lme_36_raster_sumat_overlap <- crop(LME_36_raster, extent(southern_sumat_raster))

#extend extent of lme_36_raster_sumat_overlap
lme_36_raster_sumat_full_extent <- extend(lme_36_raster_sumat_overlap, extent(southern_sumat_extent))

#reclassify all values as 
LME_36_raster_5s <- reclassify(lme_36_raster_sumat_full_extent, c(-Inf, Inf, 5))

sum_westind_sumat <- sum(LME_36_raster_5s,southern_sumat_raster, na.rm = T)

southern_sumatra_minus_36 <- reclassify(sum_westind_sumat, c(-Inf,0,NA, 1,1,1, 2,Inf,NA))


#Outer Phillipines Extent
4.359706039079976, 126.20429600316511
20.60413268328657, 121.01874933852636
18.950075222286138, 126.20429600316511
5.323042022983101, 130.68671769632746
...
east_ind <- c(42, 43, 44, 45, 39, 34, 38) #AND ADD SUMATRA 
...
```{r}
#east indian ocean
east_ind_spdf_mask_full <- merge(east_ind_spdf_mask_1s, southern_sumatra_minus_36)
...
west_pac_sorth_mod <- lm(data = west_pac_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(west_pac_sorth_mod)
```
...
east_pac_sorth_mod <- lm(data = east_pac_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(east_pac_sorth_mod)
```
...
west_atl_sorth_mod <- lm(data = west_atl_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(west_atl_sorth_mod)
```
...
east_atl_sorth_mod <- lm(data = east_atl_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(east_atl_sorth_mod)
```
...
west_ind_sorth_mod <- lm(data = west_ind_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(west_ind_sorth_mod)
```
...
east_ind_sorth_mod <- lm(data = east_ind_shelf_areas[latitude_end < 0], area_rasterarea ~ abs(latitude_start))
summary(east_ind_sorth_mod)
```
