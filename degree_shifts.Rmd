...
east_pac_spdf_shift <- spTransform(east_pac_spdf, CRS(newCRS_west))
...
for (i in 1:length(region_names[3:6])) {
#crop bathy layer to LME subset
  raster_extent <-
...
for (i in 1:length(region_names[1:2])) {
#crop bathy layer to LME subset
  raster_extent <-
       crop(etopo_shelf_raster_180, extent(get(paste0(region_names[i], "_agg"))))
...
west_pac_north_latitudes <- seq(0, ymax(west_pac_spdf_shift_mask), by = 0.66)
west_pac_south_latitudes <- seq(-0.66, ymin(west_pac_spdf_shift_mask), by = -0.66)
...
east_pac_north_latitudes <- seq(0, ymax(east_pac_spdf_shift_mask), by = 0.66)
east_pac_south_latitudes <- seq(-0.66, ymin(east_pac_spdf_shift_mask), by = -0.66)
...
north_extent <- c(xmin(west_atl_spdf_mask), xmax(west_atl_spdf_mask), 0, ymax(west_atl_spdf_mask))
...
#all chunks for east pacific
west_atl_north_latitudes <- seq(0, ymax(west_atl_spdf_mask), by = 0.66)
west_atl_south_latitudes <- seq(-0.66, ymin(west_atl_spdf_mask), by = -0.66)
...
#all chunks for east pacific
east_atl_north_latitudes <- seq(0, ymax(east_atl_spdf_nobuf_mask), by = 0.66)
east_atl_south_latitudes <- seq(-0.66, ymin(east_atl_spdf_nobuf_mask), by = -0.66)
...
#all chunks for east pacific
west_ind_north_latitudes <- seq(0, ymax(west_ind_spdf_mask), by = 0.66)
west_ind_south_latitudes <- seq(-0.66, ymin(west_ind_spdf_mask), by = -0.66)
...
#all chunks for east pacific
east_ind_north_latitudes <- seq(0, ymax(east_ind_spdf_mask), by = 0.66)
east_ind_south_latitudes <- seq(-0.66, ymin(east_ind_spdf_mask), by = -0.66)
...
save(east_ind_shelf_areas, west_ind_shelf_areas, east_atl_shelf_areas, west_atl_shelf_areas, east_pac_shelf_areas, west_pac_shelf_areas ,file = "shelf_areas_by_0.66latitude.RData")
