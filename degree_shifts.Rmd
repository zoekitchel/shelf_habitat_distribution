...
Second part of paper, if we move away from equator by 0.5˚ degree, how much habitat do we gain or lost
...
equalareaprojection<- crs(" +proj=eqearth +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs +towgs84=0,0,0 ")

LME_spdf.EA <- spTransform(LME_spdf, equalareaprojection)
...
Note that I am using the [equal area projection](https://www.r-bloggers.com/2018/09/quick-hit-using-the-new-equal-earth-projection-in-r/)
```{r bathy to raster}

etopo_shelf_raster <- rasterFromXYZ(etopo_shelf_df, crs = equalareaprojection)

crs(etopo_shelf_raster)
extent(etopo_shelf_raster)
crs(LME_spdf.EA)
extent(LME_spdf.EA)

packageVersion("proj4")
...
LMEs_extent <- crop(etopo_shelf_raster, extent(LME_spdf.EA))
...
0.5333˚ is representative of decadal shifts, but, for better visualization let's go with 1˚

I will put areas into 1˚ Bins (180 total degrees, so 180/1=180)
...
I am going to leave out Antarctica (61) and the Arctic (64) as Antarctica drowned out patterns in lower latitudes and Arctic doesn't have much area below 2000m to begin with. 
...
west_pac_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% west_pac,]
east_pac_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% east_pac,]
west_atl_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% west_atl,]
west_ind_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% west_ind,]
east_atl_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% east_atl,]
east_ind_spdf <- LME_spdf.EA[(LME_spdf.EA$LME_NUMBER) %in% east_ind,]

#for subregions that span 360, we need to change CRS a bit
newCRS_west <- "+proj=eqearth +datum=WGS84 +lon_wrap=180"
...
newCRS_east <- "+proj=eqearth +datum=WGS84 +lon_wrap=180"
...
west_pac_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000]

west_pac_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

west_pac_shelf_areas_highlight <- west_pac_shelf_areas[change_above_40 != 0,]

west_pac_shelf_areas_stats <- table(west_pac_shelf_areas[,.(change_above_40)])
```
...
east_pac_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000]

east_pac_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

east_pac_shelf_areas_highlight <- east_pac_shelf_areas[change_above_40 != 0,]

east_pac_shelf_areas_stats <- table(east_pac_shelf_areas[,.(change_above_40)])
```
...
west_atl_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000]

west_atl_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

west_atl_shelf_areas_highlight <- west_atl_shelf_areas[change_above_40 != 0,]

west_atl_shelf_areas_stats <- table(west_atl_shelf_areas[,.(change_above_40)])
```
...
east_atl_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000]

east_atl_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

east_atl_shelf_areas_highlight <- east_atl_shelf_areas[change_above_40 != 0,]

east_atl_shelf_areas_stats <- table(east_atl_shelf_areas[,.(change_above_40)])
```
...
west_ind_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000]

west_ind_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

west_ind_shelf_areas_highlight <- west_ind_shelf_areas[change_above_40 != 0,]

west_ind_shelf_areas_stats <- table(west_ind_shelf_areas[,.(change_above_40)])
```
...
east_ind_shelf_areas[, percent_change := (area-shift(area, type = "lag"))/shift(area, type = "lag")][,area_1000s := area/1000][,area_1000s := area/1000]

east_ind_shelf_areas[, change_above_40 := ifelse(percent_change>=0.4, 1, ifelse(percent_change<=-0.4, -1, 0))]

east_ind_shelf_areas_highlight <- east_ind_shelf_areas[change_above_40 != 0,]

east_ind_shelf_areas_stats <- table(east_ind_shelf_areas[,.(change_above_40)])
```
...
(area_latitude_east_ind  <- ggplot() +
  ...
  geom_rug(data = east_ind_shelf_areas_highlight, aes(x = latitude_start, color = as.factor(change_above_40))) +
 labs(x = "Latitude", y = expression(paste("Area (1000s of ", km^{2},")"))) +
  scale_color_discrete(name = "Instances of 40%\n Habitat Change", labels = c("Contraction", "Expansion")) +
...
area_latitude_west_atl  <- ggplot() +
  ...
  geom_rug(data = west_atl_shelf_areas_highlight, aes(x = latitude_start, color = as.factor(change_above_40))) +
 labs(x = "Latitude", y = expression(paste("Area (1000s of ", km^{2},")"))) +
  scale_color_discrete(name = "Instances of 40%\n Habitat Change", labels = c("Contraction", "Expansion")) +
...
area_latitude_east_atl  <- ggplot() +
  ...
  geom_rug(data = east_atl_shelf_areas_highlight, aes(x = latitude_start, color = as.factor(change_above_40))) +
 labs(x = "Latitude", y = expression(paste("Area (1000s of ", km^{2},")"))) +
  scale_color_discrete(name = "Instances of 40%\n Habitat Change", labels = c("Contraction", "Expansion")) +
...
area_latitude_east_pac  <- ggplot() +
  ...
  geom_rug(data = east_pac_shelf_areas_highlight, aes(x = latitude_start, color = as.factor(change_above_40))) +
 labs(x = "Latitude", y = expression(paste("Area (1000s of ", km^{2},")"))) +
  scale_color_discrete(name = "Instances of 40%\n Habitat Change", labels = c("Contraction", "Expansion")) +
...
(area_latitude_west_pac <- ggplot() +
  ...
  geom_rug(data = west_pac_shelf_areas_highlight, aes(x = latitude_start, color = as.factor(change_above_40))) +
 labs(x = "Latitude", y = expression(paste("Area (1000s of ", km^{2},")"))) +
  scale_color_discrete(name = "Instances of 40%\n Habitat Change", labels = c("Contraction", "Expansion")) +
...
How many experience 'significant' changes in habitat (at least 40% change from one bin to another from Yin et al 2017: Overall, we suggest the maximum permissible habitat loss threshold to be between 0–40%, depending on the pre-disturbed occupancy (or abundance) of a species.)

Bin shifts --> contractions (loss of 40%) versus expansions (gain of 40%) versus neutral
...
ggplot(data = significant_changes.long, aes(x="", y = value, fill = change_type)) +
  ...
  scale_fill_manual(values = c("darksalmon", "azure2", "cyan3"), name = "Habitat Change", labels = c(">= 40% Contraction", "<40% Change", ">= 40% Expansion")) +
  facet_wrap(~region) +
  geom_text(aes(label = paste0(round(value*100,1),"%")), position = position_stack(vjust = 0.6), size = 2.5) +
  scale_x_discrete(expand = c(0,0)) +
  blank_theme +
  theme(axis.text.x=element_blank())

ggsave(filename = "habitatloss_gain_4-.pdf")
```
