...
LME_bathy_statistics[, lme_name := as.factor(V1)][, lme_number := as.numeric(V1)][,lme_area := as.numeric(V1)][, dip_test := as.numeric(V1)][, dip_p_value := as.numeric(V1)][, skew := as.numeric(V1)][, kurtosis := as.numeric(V1)][, mean_elev := as.numeric(V1)]
...
for (i in 1:nrow(LME_bathy_statistics)) {
  ...
  lme_maps[[i]] <- ggplot(LME_dt) +
    ...
    annotate("text", x=Inf, y = max(LME_reduced_df$lat)+1, hjust=1, label = paste0("LME ", LME_bathy_statistics[i,lme_number], ": ", LME_bathy_statistics[i,lme_name]), fontface = "bold", size = 4)
  ...
  LME_bathy_statistics[i,"mean_elev"] <- mean
  
  max_area <- max(LME_bathy_bydepth_complete$area)
  
  #full plot
  lme_areaplots[[i]] <- plot +
    annotate("text", x=Inf, y = max_area-max_area/20, hjust=1, label = paste0(LME_bathy_statistics[i,lme_name]), fontface = "bold", size = 3) +
    annotate("text", x=Inf, y = max_area-2*(max_area/20), hjust=1, label = paste0("Dip Test: ", signif(diptest$statistic,2), " P-value = ", signif(p.value, 2)), size = 3) +
    annotate("text", x=Inf, y = max_area-3*(max_area/20), hjust=1, label = paste0("Skew: ", round(skew,2)), size = 3) +
    annotate("text", x=Inf, y = max_area-4*(max_area/20), hjust=1, label = paste0("Kurtosis: ",round(kurtosis,2)), size = 3) +
    annotate("text", x=Inf, y = max_area-5*(max_area/20), hjust=1, label = paste0("Mean: ",round(mean,2), " m"), size = 3) +
    geom_vline(xintercept = mean, color = "red")
...
BIG Qs to ASK MALIN
 ...
 - should I instead go with 200 m like Hailey did?