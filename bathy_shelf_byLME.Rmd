...
#full plot
  lme_areaplots_classified[[i]] <- lme_areaplots[[i]] +
    ...
    annotate("text", x=Inf, y = max_y-3*max_y/20, hjust=1, label = paste0("Dip Test: ", signif(diptest$statistic,2), " P-value = ", signif(p.value, 2)), size = 2) +
    annotate("text", x=Inf, y = max_y-4*max_y/20, hjust=1, label = paste0("Skew: ", round(skew,2)), size = 2) +
#    annotate("text", x=Inf, y = max_area-4*(max_area/20), hjust=1, label = paste0("Kurtosis: ",round(kurtosis,2)), size = 2) +
    annotate("text", x=Inf, y = max_y-5*max_y/20, hjust=1, label = paste0("Mean: ",round(mean,2), " m"), size = 2)
...
hourglass/multimodal     mid_dominant             none                      shallow_dominant 
       44 (70%)                8 (13%)               1 (arctic, will delete)          13 (20%) 
```{r how many of each type}
table(LME_bathy_statistics$type)
44/63 #multimodal
8/63 #mid-dominant
13/63 #shallow-dominant
```
