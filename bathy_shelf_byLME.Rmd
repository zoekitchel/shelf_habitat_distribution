...
for (i in 1:nrow(LME_bathy_statistics)) {
  ...
  diptest <- dip.test(LME_bathy_bydepth_complete$depth, simulate.p.value = TRUE, B = 2000)
  ...
  mean <- -1*cellStats(LME_mask, "mean", na.rm = T)
  LME_bathy_statistics[i,"mean_elev"] <- mean
  
  max_area <- max(LME_bathy_bydepth_complete$area)
  
  #full plot
  lme_densityplots[[i]] <- plot +
    annotate("text", x=Inf, y = max_area-max_area/20, hjust=1, label = paste0(LME_bathy_statistics[i,lme_name]), fontface = "bold") +
    annotate("text", x=Inf, y = max_area-2*(max_area/20), hjust=1, label = paste0("Dip Test: ", signif(diptest$statistic,2), " P-value = ", signif(p.value))) +
    annotate("text", x=Inf, y = max_area-3*(max_area/20), hjust=1, label = paste0("Skew: ", round(skew,2))) +
    annotate("text", x=Inf, y = max_area-4*(max_area/20), hjust=1, label = paste0("Kurtosis: ",round(kurtosis,2))) +
    annotate("text", x=Inf, y = max_area-5*(max_area/20), hjust=1, label = paste0("Mean: ",round(mean,2), " m")) +
    geom_vline(xintercept = mean, color = "red")
...
#plots to pdf
library(gridExtra)
ggsave("arranged_maps.pdf", marrangeGrob(grobs = lme_maps, ncol=2, nrow = 3))
...
TO DO:
if LME spans -180-+180, we need to find better way to plot (multiply by -1?)
...
For distributions with a dip value ≤0.01, we assigned those with a Type-I skewness ≥0.5 to pyramid, those with skewness ≤ −0.5 to inverse pyramid, and the remainder to diamond, representing those with approximately normal distributions. 
...
LME_bathy_statistics[, deep_dominant := ifelse(skew >= 0.5, T, F)]

LME_bathy_statistics[, shallow_dominant := ifelse(skew <= -0.5, T, F)]

LME_bathy_statistics[, mid_dominant := ifelse(abs(skew) <0.05, T, F)]

#negative kurtosis (no tails)
LME_bathy_statistics[, uniform := ifelse(kurtosis < -1, T, F)]
