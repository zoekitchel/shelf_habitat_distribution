...
LME_bathy_statistics[, lme_name := as.factor(V1)][, lme_number := as.numeric(V1)][,lme_area := as.numeric(V1)][, dip_test := as.numeric(V1)][, dip_p_value := as.numeric(V1)][, skew := as.numeric(V1)][, kurtosis := as.numeric(V1)][, mean_depth := as.numeric(V1)][, max_depth := as.numeric(V1)][, center_latitude := as.numeric(V1)][, dip_test_density_kernel := as.numeric(V1)][, dip_p_value_density_kernel := as.numeric(V1)][, skew_density_kernel := as.numeric(V1)][, kurtosis_density_kernel := as.numeric(V1)][, mean_depth_density_kernel := as.numeric(V1)][, max_depth_density_kernel := as.numeric(V1)]
...
for (i in 11:nrow(LME_bathy_statistics)) {
...
antarctica <- c(61)
arctic <- c(64) #likely exclude because most is 2000m+
...
```{r PCA}
library(ggfortify)
LME.pca <- prcomp(LME_bathy_statistics[,.(lme_area, mean_depth, center_latitude)], scale. = T)


LME.plot_type <- autoplot(LME.pca, data = LME_bathy_statistics, colour = 'type',
                          loadings = TRUE, loadings.colour = 'black',
                          loadings.label = TRUE, loadings.label.size = 4, loadings.la) + theme_classic()
...
hourglass/multimodal     mid_dominant             none                      shallow_dominant 
       44 (70%)                1 (2%)               1 (arctic, will delete)          20 (32%) 
