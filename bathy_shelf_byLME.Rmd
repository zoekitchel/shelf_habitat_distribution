...
baltic_bathy_coordinates[,depth := -(layer)]


ggplot(data = baltic_bathy_coordinates[!is.na(depth),], aes(x = x, y = y, color = depth)) +
...
Histograms of digital elevation models
```{r histogram}
ggplot(data = baltic_bathy_coordinates[!is.na(depth),], aes(x = depth, fill = depth)) +
...
diptest <- dip.test(baltic_bathy_coordinates$layer, simulate.p.value = FALSE, B = 2000)
...
```{r skew}
skew <- skewness(baltic_bathy_coordinates$layer, na.rm = T)
```
...
```{r kurtosis}
kurtosis <- kurtosis(baltic_bathy_coordinates$layer, na.rm = T)
```
