...
```{r list of values in raster greenland}
    #list of values within raster
...
```{r area for Polygon instead greenland}
...
Next, I need to construct hypsometric plots. One problem is that a value of 30 m in one cell does not necessarily mean the same as a 30 m value in another cell if those cells are different sizes. Can I somehow weight by cell size?
```{r list of values in raster beaufort}
    #list of values within raster
...
Area for polygon instead
```{r area for Polygon instead beaufort}
...
```{r calculating raster size Canadian High Arctic North Greenland}  
  #size of raster cells, and subsequently raster
  #cell size varies quite tremendously across high_arctic Sea!! (1.4 km^2 to 3.5 km^2)
  #get sizes of all cells in raster [km2]
    ...
    #compute area [km2] of all cells in geo_raster
   high_arctic_raster_area<-length(cell_size_high_arctic)*median(cell_size_high_arctic)
...
LME_high_arctic.EA@data

```

Frequency table to vector?

Rounding to generate values
```{r rounding to generate raw values}

LME_high_arctic_data <- data.table(LME_high_arctic.EA@data)

LME_high_arctic_data[,prop_area :=  area_m/sum(area_m)]

LME_high_arctic_data[,area_rounddown := round(prop_area * 100000000, 0)]

#empty vector
LME_high_arctic_data_vector <- vector()

#populate vector
for (i in 1:nrow(LME_high_arctic_data)){
add <- rep(LME_high_arctic_data[i,depth], LME_high_arctic_data[i,area_rounddown])
...
#statistics
dip.test_generatedvalues_high_arctic <- dip.test(LME_high_arctic_data_vector)

```

Calculating statistics for polygon of Canadian High Arctic North Greenland
...
#statistics
#diptest?
dip.test_generatedvalues_high_arctic$statistic[1]
  p.value_high_arctic_polygon <- dip.test_generatedvalues_high_arctic$p.value
p.value_high_arctic_polygon
  skew_high_arctic_polygon <- skew.freqtab(high_arctic_polygon_statistics)
skew_high_arctic_polygon
  kurtosis_high_arctic_polygon <- kurt.freqtab(high_arctic_polygon_statistics)
kurtosis_high_arctic_polygon
  mean_high_arctic_polygon <- mean(high_arctic_polygon_statistics)
mean_high_arctic_polygon
  max_depth_high_arctic_polygon <- max(high_arctic_polygon_statistics)
max_depth_high_arctic_polygon
  median_depth_high_arctic_polygon <- median(high_arctic_polygon_statistics)
median_depth_high_arctic_polygon

diptest_high_arctic_raster[1]
dip.test_generatedvalues_high_arctic$statistic[1]
p.value_high_arctic_raster
p.value_high_arctic_polygon
skew_high_arctic_raster
...
(plot_higharctic_polygon <- ggplot(data = LME_high_arctic.EA@data, aes(x = depth, y = area_km, fill = depth)) +
 ...
 # geom_smooth(method = "loess", se = F, color = "black") +
  ...
  annotate("text", x = 700, y = 1550, label = paste0("Dip Test: ", signif(dip.test_generatedvalues_high_arctic$statistic,2), " P-value = ", signif(p.value))) +
  annotate("text", x = 700, y = 1450, label = paste0("Skew: ", round(skew_high_arctic_polygon,2))) +
#  annotate("text", x = 700, y = 1350, label = paste0("Kurtosis: ",round(kurtosis_polygon,2))) +
  annotate("text", x = 700, y = 1250, label = paste0("Mean: ",round(mean_high_arctic_polygon,2), " m")) +
...
(plot_higharctic_raster <- ggplot(data = high_arctic_bathy_bydepth_complete, aes(x = depth, y = area, fill = depth)) +
  ...
  annotate("text", x = 700, y = 1550, label = paste0("Dip Test: ", signif(diptest_high_arctic_raster$statistic,2), " P-value = ", signif(p.value))) +
