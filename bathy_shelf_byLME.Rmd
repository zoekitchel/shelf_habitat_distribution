...
#full plot
  lme_areaplots_classified[[i]] <- lme_areaplots[[i]] +
    geom_vline(xintercept = mean, color = "red") +
    annotate("text", x=Inf, y = max_area-max_area/20, hjust=1, label = paste0("LME ", LME_bathy_statistics[i,lme_number], ": ", LME_bathy_statistics[i,lme_name]), fontface = "bold", size = 2) +
        annotate("text", x=Inf, y = max_area-2*max_area/20, hjust=1, label = paste0("Classification: ", LME_bathy_statistics[i,type]), size = 2) +
    annotate("text", x=Inf, y = max_area-3*(max_area/20), hjust=1, label = paste0("Dip Test: ", signif(diptest$statistic,2), " P-value = ", signif(p.value, 2)), size = 2) +
    annotate("text", x=Inf, y = max_area-4*(max_area/20), hjust=1, label = paste0("Skew: ", round(skew,2)), size = 2) +
