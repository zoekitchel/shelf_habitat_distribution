...
LME_bathy_statistics[, lme_name := as.factor(V1)][, lme_number := as.numeric(V1)][,lme_area := as.numeric(V1)][, dip_test := as.numeric(V1)][, dip_p_value := as.numeric(V1)][, skew := as.numeric(V1)][, kurtosis := as.numeric(V1)][, mean_depth := as.numeric(V1)][, center_latitude := as.numeric(V1)][, dip_test_density_kernel := as.numeric(V1)][, dip_p_value_density_kernel := as.numeric(V1)][, skew_density_kernel := as.numeric(V1)][, kurtosis_density_kernel := as.numeric(V1)][, mean_depth_density_kernel := as.numeric(V1)]
...
for (i in 1:nrow(LME_bathy_statistics)) {
  ...
  diptest_density_kernel <- dip.test(LME_bathy_depth_list_pos, simulate.p.value = TRUE, B = 2000)
  ...
  skew_density_kernel <- skewness(LME_bathy_depth_list_pos, na.rm = T)
  LME_bathy_statistics[i,"skew_density_kernel"] <- skew_density_kernel
  kurtosis_density_kernel <- kurtosis(LME_bathy_depth_list_pos, na.rm = T)
  LME_bathy_statistics[i,"kurtosis_density_kernel"] <- kurtosis_density_kernel
  mean_density_kernel <- mean(LME_bathy_depth_list_pos)
  ...
  lme_densityplots[[i]] <- ggplot() +
    ...
    annotate("text", x=Inf, y = max_density+max_density/50, hjust=1, label = paste0(LME_bathy_statistics[i,lme_name]), fontface = "bold", size = 2) +
    theme_classic() +
...
LME.pca <- prcomp(LME_bathy_statistics[,.(lme_area, mean_depth)])
LME.plot <- autoplot(LME.pca)

library(ggbiplot)

ggbiplot(LME.pca)#groups
```
