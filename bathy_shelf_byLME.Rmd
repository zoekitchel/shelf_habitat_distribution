...
LME_bathy_statistics[, lme_name := as.factor(V1)][, lme_number := as.factor(V1)][,lme_area := as.numeric(V1)][, dip_test := as.numeric(V1)][, dip_p_value := as.numeric(V1)][, skew := as.numeric(V1)][, kurtosis := as.numeric(V1)][, mean_elev := as.numeric(V1)]
LME_bathy_statistics[, V1 := NULL]

lme_maps <- list()
lme_densityplots <- list()

for (i in length(1:nrow(LME_bathy_statistics))) {
  ...
  #get sizes of all cells in raster [km2]
    ...
    LME_bathy_statistics[i, lme_area] <- LME_single$bathy_raster_area
  ...
  LME_bathy_statistics[i,dip_test] <- diptest
  p.value <- diptest$p.value
  ...
  LME_bathy_statistics[i,mean] <- mean
